...
export type RefListKeys<T> = {
    [K in keyof T]-?: T[K] extends RefList ? K : never;
}[keyof T];
export type RefListValue<List> = List extends RefList<infer V> ? V : never;
export type RefSetKeys<T> = {
    [K in keyof T]-?: T[K] extends RefSet ? K : never;
}[keyof T];
export type RefSetValue<Set> = Set extends RefSet<infer V> ? V : never;
export type RefMapKeys<T> = {
    [K in keyof T]-?: T[K] extends RefMap ? K : never;
}[keyof T];
export type RefMapValue<Map> = Map extends RefMap<infer V> ? V : never;
export type RefCollectionValue<Collection> = Collection extends RefList<infer T> | RefSet<infer T> | RefMap<infer T> ? T : never;
